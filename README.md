...
A food rating website for the TIFRH canteen. Anyone can open it and read ratings and comments.
Only people with an `@tifrh.res.in` email can rate, comment or upload photos (they sign in when they try).
...
| File | What it is |
|---|---|
| `index.html` | All the pages (login, home, meal list, item, committee, gallery, admin). Only one is shown at a time. |
| `style.css` | Colours and layout. Light-mode colours are at the top, dark-mode colours right below. |
| `app.js` | All logic: login, ratings, polls, announcements, slider, committee, gallery, admin. |
| `firebase-config.js` | **You edit this.** Your Firebase keys and the admin emails. |
| `firestore.rules` | Security rules. **You paste this into Firebase** every time it changes. |
| `starter-menu.js` | Ready-made list of ~70 common items. The Admin page's **Add starter menu** button loads it. Fix the MRPs first. |
| `images/` | Optional photos: `canteen.jpg` (big photo at the top of the home page) and the 5 category cards: `breakfast.jpg`, `lunch.jpg`, `dinner.jpg`, `snacks.jpg`, `fruits.jpg`. |

Canteen timings shown on the category cards are written directly in `index.html` (search for "EDIT THE TIMINGS").

## What admins manage, and where

| Thing | Where |
|---|---|
| Food items, prices, days, photos, thali parts | Admin page (☰ menu → Admin) |
| Announcements (yellow box on the home page) | Admin page |
| Polls ("Have your say" on the home page) | Admin page: create, close / reopen, delete |
| Committee members (name, position, photo) | Canteen Committee page, while signed in as admin |
| Gallery photos | Gallery page, while signed in as admin |

The announcement box and the poll box only appear on the home page when there is something in them.
...
- **Names on reviews**: signed-in TIFRH people see the reviewer's name (the part of the email before `@`).
  Visitors who are not signed in see "TIFRH member" instead. The names live in a separate
  `reviewNames` collection that the rules only let TIFRH logins read.
